...
def simulate_seir_hcd(params: SEIRHCDParams, days: float, dt: float = 1.0, beta_time_fn=None):
    """
    ...
    flow_new_exposed = np.zeros(n_steps)   # new exposures (infections)
    flow_E_to_I = np.zeros(n_steps)
    flow_I_to_H = np.zeros(n_steps)
    flow_I_to_R = np.zeros(n_steps)
    flow_H_to_C = np.zeros(n_steps)
    flow_H_to_R = np.zeros(n_steps)
    flow_C_to_R = np.zeros(n_steps)
    flow_C_to_D = np.zeros(n_steps)
    ...
    df = pd.DataFrame({
        ...
        # include instantaneous rates for debugging/inspection
